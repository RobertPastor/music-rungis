...
(function() {
    ...
    DayPilotCalendar.Calendar = function(id) {
        ...
        this.visible = true;
        // Robert - 21 Mai 2016 - manage french Bank Holidays
        this.frenchBankHolidays = null;
        ...
        this.businessHoursSpan = function() {
                if (this.businessBeginsHour > this.businessEndsHour) {
					var businessSpan = durationHours - this.businessBeginsHour + this.businessEndsHour;
                    return durationHours - this.businessBeginsHour + this.businessEndsHour;
                }
                else {
        ...
        this.rowCount = function() {
            //return 48;
			return durationHours * HourParts;
        };
        ...
        // Robert - 22 mai 2016
        // tells true or false if the given date is a bank holiday
        this._isBankHolidays = function(date) {
        	if (this.frenchBankHolidays != null) {
        		for (var i = 0; i < this.frenchBankHolidays.length; i++) {
        			
        			var bankHoliday = this.frenchBankHolidays[i];
        			var bankHolidayDate = new DayPilot.Date(bankHoliday.date);
        			//console.log ( 'bank holiday - ' + bankHolidayDate);
        			if (bankHolidayDate == date) {
        				return true;
        			}
        		}
        	}
        ...
        // Robert - 22 mai 2016
        // returns the name of the bank holidays
        this._getBankHolidaysName = function(date) {
        	if (this.frenchBankHolidays != null) {
        		for (var i = 0; i < this.frenchBankHolidays.length; i++) {
        ...
        this._createDaysViewColumns = function() {
            ...
            for (var i = 0; i < days; i++) {
                ...
                // Robert - display 1er followed by the month
                column.Name = column.Start.toString(format.split(' ')[0], resolved.locale());
            	column.Name += ' ';
                var day = column.Start.getDay();
                if (day < 10) {
                	column.Name += column.Start.toString(format.split(' ')[1], resolved.locale())[1];
                	if (day == 1) {
                    	column.Name += 'er';
                	} 
                } else {
                	column.Name += column.Start.toString(format.split(' ')[1], resolved.locale());
                }
            	column.Name += ' ';
                column.Name += column.Start.toString(format.split(' ')[2], resolved.locale());
            	column.Name += ' ';
                column.Name += column.Start.toString(format.split(' ')[3], resolved.locale());

                column.InnerHTML = column.Name;
                // Robert - 22 mai 2016 - show bank holiday
                if (this._isBankHolidays(column.Start)) {
                	column.InnerHTML = this._getBankHolidaysName(column.Start);
                	//console.log ( ' add name of the bank holiday ');
                }
                columns.push(column);
            }
        ...
        this._drawEvent = function(e) {
            ...
            *
            ...
            div.style.height = Math.max(e.part.height, HourParts) + 'px';
            ...
            div.onmousedown = function (ev) {
				...
				else if (button == 3 && calendar.eventMoveHandling !== 'Disabled') {
					// Robert - console.log ( ' right click mouse bouton clicked ' );
					console.log ( ' right click mouse down bouton clicked ' );
					ev.preventDefault();
				}
            ...
            if (e.client.barVisible()) {
				//console.log ( ' on mouse down event client bar is visible ' );
        ...
        this.createHourRow = function(table, i) {
            var height = (this.cellHeight * HourParts);
        ...
        this.getScrollableHeight = function() {
            switch (this.heightSpec) {
                case "Full":
                    return (durationHours * HourParts * this.cellHeight);    
                case "BusinessHours":
                    var dHours = this.businessHoursSpan();
                    return dHours * this.cellHeight * HourParts;
                ...
                for (var j = 0; j < cl; j++) {
                    ...
                    if (create) {
                        ...
                        if (!this.cssOnly) {
                            ...
                            // Robert - add mouse down event
                            inner.onmousedown = function (ev) {
                            	ev = ev || window.event;
                    			var button = ev.which || ev.button;
                    			if (button == 3) {
                    				var originalMouse = DayPilot.mc(ev);
                    				console.log( 'right click mouse down on cell inner');
                    			}
                    			ev.preventDefault();
                            }
        ...
        this._enableScrolling = function() {
            ...
            if (scrollpos === 'Auto') {
                if (this.heightSpec === "BusinessHours") {
                    scrollpos = HourParts * this.cellHeight * this.businessBeginsHour;
                }
                else {
                    scrollpos = 0;
                }
        ...
        // Robert - 30 April 2016
		this.events.get = function(id) {
			//console.log ( ' events get ' );
			if (calendar.events != undefined) {
				if (calendar.events.list != undefined) {
					for (var i = 0; i < calendar.events.list.length; i++) {
		                var e = calendar.events.list[i];
						//console.log ( e.id );
						if ( e.id === id ) {
							//console.log ( ' event found ... ' );
							return new DayPilot.Event(e, this);
						}
					}
				}
			}
		...
		// Robert - 15 May 2019
		this.isoWeekHasEvents = function ( isoWeekInt ) {
			var ret = false;
			// check if input is integer
			if ( !isNaN( isoWeekInt ) && (typeof isoWeekInt === 'number') && ( ( isoWeekInt % 1 ) === 0) ) {
				if (this.events != undefined) {
					if (this.events.list != undefined) {
						for (var i = 0; i < this.events.list.length; i++) {
			                var e = this.events.list[i];
			                var eventStartDate = e.start;
			                //console.log ( eventStartDate.weekNumberISO() );
			                //console.log ( typeof eventStartDate.weekNumberISO() );
			                //console.log ( eventStartDate.weekNumberISO() === isoWeekInt );
			                if ( eventStartDate.weekNumberISO() === isoWeekInt ) {
			                	ret = true;
			                }
						}
					}
            ...
            for(var i = 0; i < this.columns.length; i++) {
                ...
                for (var j = 0; j < length; j++) {
                    ...
                    if (belongsHere) {
                        ...
                        ep.part.barHeight = Math.max(bottom - top - HourParts, 1);  // minimum 1
        ...
        this._saveScrollHour = function() {
            ...
            var pos = top / ( HourParts *calendar.cellHeight);
        ...
        this._restoreScrollHour = function() {
            ...
            if (calendar._config.scrollHour) {
                scrollpos = HourParts * calendar.cellHeight * calendar._config.scrollHour;
